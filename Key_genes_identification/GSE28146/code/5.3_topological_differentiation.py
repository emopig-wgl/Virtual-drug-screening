...
class network_complex:
    def __init__(self, path, path_deg) -> None:
        ...
        self.edges = {}
        for i in range(
                len(df)):  # ，读取df数据框中的每一行，提取节点1（#node1）、节点2（node2）和它们之间的组合分数（combined_score）。然后将每个边（由节点1和节点2组成的字符串，格式为'{node1}_{node2}'）及其对应的分数保存到实例变量self.edges字典中。
    ...
    def rips(self, start=0, end=0.3, stride=0.03):
        ...
        intervals = np.arange(start, end + stride,
                              stride)  # 创建一个数组，包含从 start 到 end（包含）的等间距值，步长为 stride。这些值将用于定义Rips复形的参数。
        ...
        rips_complex = gudhi.RipsComplex(distance_matrix=matrixA,
                                         max_edge_length=1)  # 用 gudhi.RipsComplex 创建一个Rips复形对象，传入 matrixA 作为距离矩阵，并将最大边长设置为1。
        Ph = rips_complex.create_simplex_tree(
            max_dimension=2).persistence()  # rips_complex.create_simplex_tree 方法创建一个单纯复形树，并设置最大维度为2（这意味着它将计算0维、1维和2维的持久性同调）。然后，调用 .persistence() 方法计算持久性同调。
    ...
    def get_result(self, start=0, end=0.3, stride=0.03):
        ...
        for i, node in enumerate(self.nodes):
            ...
            score = np.linalg.norm(self.remove_node(i, start=start, end=end,
                                                    stride=stride) - original)  # 使用 np.linalg.norm 函数计算移除节点后得到的持久性贝蒂数与原始持久性贝蒂数之间的欧几里得距离（即两个向量之间的差异程度），这个距离作为节点移除的得分。
    ...
    def get_result_static(self, dim=1):
        ...
        for i, node in enumerate(self.nodes):
            # score = self.remove_node(i)
            score = np.linalg.norm(self.static_remove_node(i,
                                                           dim=dim) - original)  # np.linalg.norm 函数计算移除节点后得到的持久性贝蒂数与原始持久性贝蒂数之间的欧几里得距离（即两个向量之间的差异程度），这个距离作为节点移除的得分
            result_dic[node] = score
            if node == 'EGR1':
                x = 1
                pass
    ...
    def get_result_lap(self, start=0, end=0.3, stride=0.03):
        ...
        for i, node in enumerate(self.nodes):
            ...
            if node == 'EGR1':
                x = 1
                pass
        return result_dic, ls1, ls2, ls, list(
            preprocessing.minmax_scale(np.array(ls1)) + preprocessing.minmax_scale(np.array(ls2)))
...
patp = 'C:/Users/lenovo/Desktop/文章4/数据集筛选/筛选重复基因/GSE28146/'
compl = network_complex(patp + f'/GSE28146_string_interactions_short_0.7.tsv', patp + '/GSE28146_DEGs_limma_split.csv')
df = pd.DataFrame()
df['node'] = compl.nodes
...
df[f'lap_barcode_0.7'] = lap[1]
df[f'lap_lamda_0.7'] = lap[2]
df[f'lap_sum_0.7'] = lap[3]
df[f'lap_sum_norm_0.7'] = lap[4]
dfs.append(df)
merged_df = reduce(lambda left, right: pd.merge(left, right, on='node'), dfs)
merged_df.to_csv('C:/Users/lenovo/Desktop/文章4/数据集筛选/筛选重复基因/GSE28146/ph_cocaine_norm_statistics_0.7.csv')
