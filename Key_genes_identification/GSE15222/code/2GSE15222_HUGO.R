...
# 2 读取已经转换好的探针表达矩阵
expr <- read.csv("GSE15222.csv", row.names = 1, check.names = FALSE)

# 3 下载对应平台
gpl <- getGEO("GPL2700", AnnotGPL = TRUE)
gpl_table <- Table(gpl)
...
# 8 保存结果
write.csv(expr_HUGO, "GSE15222_HUGO_expression.csv", row.names = FALSE)
