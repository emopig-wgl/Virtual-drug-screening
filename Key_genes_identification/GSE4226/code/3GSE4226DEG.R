...
gse <- getGEO(
  filename = "C:/Users/lenovo/Desktop/文章4/数据集筛选/GSE4226/GSE4226_series_matrix.txt/GSE4226_series_matrix.txt"
)
...
group <- ifelse(
  grepl("^AD", pdata$title),
  "AD",
  "NEC"
)

group <- factor(group, levels = c("NEC", "AD"))
...
p <- ggplot(
  ...
  theme_bw() +
  labs(
    title = "GSE4226 Differential Expression (limma)",
    x = "log2 Fold Change",
    y = "-log10(P value)"
  )

ggsave(
  filename = "GSE4226_volcano_limma.png",
  plot = p,
  width = 6,
  height = 5
)
...
ggsave(
  filename = "GSE4226_volcano_limma.pdf",
  plot = p,
  width = 6,
  height = 5
)
