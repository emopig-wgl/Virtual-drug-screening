...
# 2 读取已经转换好的探针表达矩阵
expr <- read.csv("GSE122063.csv", row.names = 1, check.names = FALSE)

# 3 下载对应平台
gpl <- getGEO("GPL16699", AnnotGPL = TRUE)
gpl_table <- Table(gpl)

# 4 提取探针→基因（HUGO）映射
mapping <- gpl_table[, c("ID", "GENE_SYMBOL")]
colnames(mapping)[2] <- "Gene symbol"  # 添加这行重命名
...
# 8 保存结果
write.csv(expr_HUGO, "GSE122063_HUGO_expression.csv", row.names = FALSE)
