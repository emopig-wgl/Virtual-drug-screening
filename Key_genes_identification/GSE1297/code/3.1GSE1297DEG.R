...
setwd("C:/Users/lenovo/Desktop/文章4/数据集筛选/GSE1297")
...
gse <- getGEO(
  filename = "C:/Users/lenovo/Desktop/文章4/数据集筛选/GSE1297/GSE1297_series_matrix.txt/GSE1297_series_matrix.txt"
)
...
group <- ifelse(
  grepl("control", pdata$title, ignore.case = TRUE),
  "control",
  "AD"
)
...
expr <- fread(
  "GSE1297_HUGO_expression_log2.csv",
  data.table = FALSE
)
...
p <- ggplot(
  ...
  theme_bw() +
  labs(
    title = "GSE1297 Differential Expression (limma)",
    x = "log2 Fold Change",
    y = "-log10(P value)"
  )

ggsave(
  filename = "GSE1297_volcano_limma.png",
  plot = p,
  width = 6,
  height = 5
)
...
ggsave(
  filename = "GSE1297_volcano_limma.pdf",
  plot = p,
  width = 6,
  height = 5
)
