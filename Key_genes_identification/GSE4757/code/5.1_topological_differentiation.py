...
#返回结果字典：最后，返回包含所有节点移除得分的 result_dic 字典。
    ...
    # 新增函数：返回持久条码 (barcode)
    # 保存 dim, birth, death
    # remove_id=None -> original
    # remove_id=i -> removed node i
    # ===============================
    def get_persistent_barcode(self, start=0, end=0.3, stride=0.03, remove_id=None):
        ...
        for simplex in Ph:
            dim = int(simplex[0])
            birth = float(simplex[1][0])
            death = float(simplex[1][1])

            if death != math.inf and (death - birth) > 0.00002:
                barcode_list.append({
                    "dimension": dim,
                    "birth": birth,
                    "death": death
...
patp = 'C:/Users/lenovo/Desktop/文章4/数据集筛选/筛选重复基因/GSE4757/'
#for cutoff in [0.15,0.4,0.7,0.9]:
compl = network_complex(patp + f'/GSE4757_string_interactions_short_0.15.tsv', patp + '/GSE4757_DEGs_limma_split.csv')
df = pd.DataFrame()
df['node'] = compl.nodes
lap = compl.get_result_lap_statistics(start=0,end=1-0.15,stride=(1-0.15)/10)
# ===============================
# 新增：保存 original 网络的 Laplacian 5统计量随过滤尺度变化
# ===============================
cutoff = 0.15
start = 0
end = 1 - cutoff
...
# lap_statistics 会输出 (len(intervals)*5,) flatten
lap_stats_flat = compl.lap_statistics(start=start, end=end, stride=stride, remove_id=None)

# reshape 成 (len(intervals), 5)
lap_stats = lap_stats_flat.reshape(len(intervals), 5)

lap_stats_df = pd.DataFrame({
    "radius": intervals,
    "sum": lap_stats[:, 0],
    "mean": lap_stats[:, 1],
    "max": lap_stats[:, 2],
...
save_path = patp + "/fig3/laplacian_statistics_original.csv"
lap_stats_df.to_csv(save_path, index=False)

print("Saved Laplacian statistics to:", save_path)

import json

# ===============================
# 新增：保存 persistent barcode
# original 和 removed TYROBP
# ===============================
cutoff = 0.15
start = 0
end = 1 - cutoff
...
# original barcode
barcode_original = compl.get_persistent_barcode(start=start, end=end, stride=stride, remove_id=None)

# removed TYROBP barcode
remove_gene = "TYROBP"
remove_id = compl.node_id[remove_gene]
barcode_removed = compl.get_persistent_barcode(start=start, end=end, stride=stride, remove_id=remove_id)

barcode_out = {
    "original": barcode_original,
    "removed_TYROBP": barcode_removed
}

barcode_save_path = patp + "/fig3/persistent_barcodes_TYROBP.json"

with open(barcode_save_path, "w") as f:
    json.dump(barcode_out, f, indent=4)

print("Saved barcode json to:", barcode_save_path)
...
df[f'lap_barcode_0.15'] = lap[1]
df[f'lap_lamda_0.15'] = lap[2]
df[f'lap_sum_0.15'] = lap[3]
df[f'lap_sum_norm_0.15'] = lap[4]
dfs.append(df)
merged_df = reduce(lambda left, right: pd.merge(left, right, on='node'), dfs)
merged_df.to_csv('C:/Users/lenovo/Desktop/文章4/数据集筛选/筛选重复基因/GSE4757/ph_cocaine_norm_statistics_0.15.csv')
