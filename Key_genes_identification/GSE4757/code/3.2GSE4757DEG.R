...
setwd("C:/Users/lenovo/Desktop/文章4/数据集筛选/GSE4757")
...
gse <- getGEO(
  filename = "C:/Users/lenovo/Desktop/文章4/数据集筛选/GSE4757/GSE4757_series_matrix.txt/GSE4757_series_matrix.txt"
)
...
group <- ifelse(
  grepl("normal", pdata$title, ignore.case = TRUE),
  "control",
  "AD"
)
...
expr <- fread(
  "GSE4757_HUGO_expression_log2.csv",
  data.table = FALSE
)
...
font_size <- 16   # 统一控制坐标标题、图例标题、图例文字大小
...
p <- ggplot(
  ...
  scale_color_manual(
    values = c("down" = "blue4", "stable" = "grey", "up" = "red3"),
    name = "Change"   # 图例标题，可改成 DEGs / Regulation 等
  ) +
  geom_vline(
...
ggsave(
  filename = "GSE4757_volcano_limma.png",
  plot = p,
  width = 6,
  height = 5,
  dpi = 300
...
ggsave(
  filename = "GSE4757_volcano_limma.pdf",
  plot = p,
  width = 6,
  height = 5
)
